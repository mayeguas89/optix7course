...
void SampleRenderer::initOptix()
{
  ...
  std::cout << GDT_TERMINAL_GREEN << "#osc: successfully initialized optix... yay!" << GDT_TERMINAL_DEFAULT
            << std::endl;
}
...
void SampleRenderer::createMissPrograms()
{
  ...
  OPTIX_CHECK(
    optixProgramGroupCreate(optixContext, &pgDesc, 1, &pgOptions, log, &sizeof_log, &missPGs[RADIANCE_RAY_TYPE]));
  if (sizeof_log > 1)
    PRINT(log);
  ...
  OPTIX_CHECK(
    optixProgramGroupCreate(optixContext, &pgDesc, 1, &pgOptions, log, &sizeof_log, &missPGs[SHADOW_RAY_TYPE]));
  if (sizeof_log > 1)
    PRINT(log);
}
...
void SampleRenderer::buildSBT()
{
  ...
  {
    // Para cada uno de los tipos de rayo normal y sombra
...
void SampleRenderer::setBackground(const vec3f& color)
{
  launchParams.bgColor = color;
}
